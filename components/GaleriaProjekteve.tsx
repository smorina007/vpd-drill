      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Disa nga projektet e realizuara me sukses në terren
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projektet.map((projekt) => (
            <div 
              key={projekt.id} 
              className="bg-white rounded-lg shadow-sm hover:shadow-md transition p-6 border border-gray-100"
            >
              <div className="flex justify-between items-start mb-3">
                <span className="text-sm font-medium text-[#256D7B]">{projekt.data}</span>
                <span className="text-xs bg-[#256D7B]/10 text-[#256D7B] px-2 py-1 rounded-full">
                  {projekt.kategoria}
                </span>
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{projekt.titulli}</h3>
              <p className="text-sm text-gray-500">📍 {projekt.lokacioni}</p>
            </div>
          ))}
        </div>
